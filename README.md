...
- 软件内选择：背景图片、音频/视频音乐来源、歌词文件、输出 MP4，并可填写歌曲名、作者和右上角水印
...
- GUI 播放器式实时预览与最终导出已分离：预览先完整解码/分轨/频谱预处理音频，再按独立 30FPS 连续播放轻量画面，默认最高 1280×720；最终导出继续使用选择的 1080p/2K/4K 与导出 FPS
...
- 软件内打开播放器式实时预览：先完成音乐解码、分轨、重采样和频谱分析，再从指定时间点连续播放到歌曲结束；正式渲染时同步打开录制预览窗口，窗口只负责显示，最终 MP4 由同源原始合成帧直接编码，避免屏幕截图式录制变糊；预览窗口右侧会显示音频处理状态、频段能量、Beat/Drop、声场宽度和多轨 stem 参数
- 软件内取消当前渲染任务，并可快速打开输出文件夹
- 软件内统计预览和完整渲染耗时
- 渲染前自动检查素材路径、文件格式、空文件、输出目录和覆盖确认
- 支持把 `.mp4` / `.mkv` / `.webm` / `.mov` 等视频作为音乐来源：选择视频时会保留原视频画面/页面，只复制视频流并替换为增强后的 HiFi 3D 音轨；背景图和歌词只用于普通音频生成 MV 模式
...
- 导出音频使用手写 DSP 空间规划 + FFmpeg 执行链：支持“全方位耳机沉浸”“人声突出”“强环绕”模式，C++ 根据虚拟声源的 360° 方位角、距离和高度自动计算 ITD 双耳时间差、IID 双耳电平差、Haas 前后深度、前后滤波、早期反射、低频居中、主声保真、近/远场、后方包围和顶部空气层；视频来源仍保留原视频画面，只替换增强后的沉浸音轨；自动检测 `firequalizer`、`crossfeed`、`stereotools`、`afftdn`、`crystalizer`，不可用时回退兼容处理
...
1. 背景图片：`.png` / `.jpg` / `.jpeg` / `.webp` / `.bmp`（普通音频生成 MV 时需要；视频来源模式不使用）
2. 音乐来源：`.mp3` / `.wav` / `.flac` / `.aac` / `.ogg` / `.m4a` / `.opus`，也可以直接选带音轨的视频：`.mp4` / `.mkv` / `.webm` / `.mov` / `.avi` / `.flv` / `.wmv`
3. 歌词文件：`.lrc`（普通音频生成 MV 时需要；视频来源模式不使用）
4. 输出路径：`.mp4`

> 视频来源模式：画面保持原视频，不重新生成 MV 页面；程序只处理并替换音轨，因此不会要求背景图和歌词，也不会应用歌词、频谱、粒子等画面特效。
...
也可以直接把素材文件拖入主窗口：图片会自动填入背景，音频或带音轨的视频会自动填入音乐来源，`.lrc`/`.txt` 会自动填入歌词，输出路径建议通过 **选择输出** 指定。
...
- 背景、音乐来源、歌词、输出路径不能为空
- 背景图片、音频/视频音乐来源、歌词文件必须存在且可读
- 音乐来源可以是普通音频，也可以是带音轨的视频文件；无音频流的视频会阻止渲染
- 文件扩展名必须属于支持列表
- 歌词文件为空会阻止渲染
